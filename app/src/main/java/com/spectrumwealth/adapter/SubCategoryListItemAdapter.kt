...
import com.spectrumwealth.utils.customdialog.CustomSubCategoryDialog

class SubCategoryListItemAdapter(
    val mainActivity: MainActivity,
    val subcategorylist: ArrayList<SubCategoryApiResponseModel.SubCategoryItem>,
    val  dialog: CustomSubCategoryDialog,
    val onitemclick: OnCategoryItemClickInterface
) : RecyclerView.Adapter<SubCategoryListItemAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.categoryname.text = subcategorylist.get(position).subcategory
        holder.categoryname.typeface = CustomTypeface(mainActivity).openSansSemiBold

        holder.rrcard.setOnClickListener {
            //onitemclick.onItemClick(subcategorylist.get(position), position)
        }
